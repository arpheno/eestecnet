...
#The function magic_open() creates a magic cookie pointer and returns it. It returns
# NULL if there was an error allocating the magic cookie. The flags argument specifies
...
#MAGIC_DEVICES
#If the file is a block or character special device, then open the device and try to
...
#MAGIC_ERROR' Treat operating system errors while trying to open files and follow
# symlinks as real errors, instead of printing them in the magic buffer.

#The magic_close() function closes the magic(5) database and deallocates any resources
#  used.

#The magic_error() function returns a textual explanation of the last error, or NULL
# if there was no error.

#The magic_errno() function returns the last operating system error number ( errno(3))
#  that was encountered by a system call.

#The magic_file() function returns a textual description of the contents of the
# filename argument, or NULL if an error occurred. If the filename is NULL, then stdin
#  is used.

#The magic_buffer() function returns a textual description of the contents of the
# buffer argument with length bytes size.

#The magic_setflags() function, sets the flags described above.

#The magic_check() function can be used to check the validity of entries in the colon
# separated database files passed in as filename, or NULL for the default database. It
#  returns 0 on success and -1 on failure.

#The magic_compile() function can be used to compile the the colon separated list of
# database files passed in as filename, or NULL for the default database. It returns 0
...
#The magic_load() function must be used to load the the colon separated list of
# database files passed in as filename, or NULL for the default database file before
# any magic queries can performed.

#The default database file is named by the MAGIC environment variable. If that
# variable is not set, the default database file name is /usr/share/file/magic.

#magic_load() adds ".mime" and/or ".mgc" to the database filename as appropriate.
#Return Values

#The function magic_open() returns a magic cookie on success and NULL on failure
# setting errno to an appropriate value. It will set errno to EINVAL if an unsupported
#  value for flags was given. The magic_load(), magic_compile(), and magic_check()
# functions return 0 on success and -1 on failure. The magic_file(), and magic_buffer(
# ) functions return a string on success and NULL on failure. The magic_error()
# function returns a textual description of the errors of the above functions,
# or NULL if there was no error. Finally, magic_setflags() returns -1 on systems that
...
for f in (
magic_buffer, magic_check, magic_close, magic_descriptor, magic_file, magic_load):
    f.errcheck = errcheck
