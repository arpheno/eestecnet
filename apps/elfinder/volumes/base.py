...
from apps.elfinder.exceptions import ElfinderErrorMessages, FileNotFoundError, \
    DirNotFoundError, PermissionDeniedError, NamedError, NotAnImageError
from apps.elfinder.utils.archivers import ZipFileArchiver
...
class ElfinderVolumeDriver(object):
    ...
    def __init__(self):
        ...
        self._options = {
            ...
            'alias': '',
            #root url, not set to disable sending URL to client (replacement for old
            # "fileURL" option)
            'URL': '',
            ...
            'copyJoin': True,
            #on upload -  if True - old file will be replaced with new one, if False
            ...
            #maximum upload file size. Set as number or string with unit - "10M",
            # "500K", "1G". NOTE - applies to each uploaded file individually
            'uploadMaxSize': 0,
            #files dates format. CURRENTLY NOT IMPLEMENTED
            'dateFormat': 'j M Y H:i',
            #files time format. CURRENTLY NOT IMPLEMENTED
            'timeFormat': 'H:i',
            #if True - every folder will be check for children folders, otherwise all
            # folders will be marked as having subfolders
            ...
            #least one of the 'hidden', 'locked', 'read' and 'write' attributes for
            # this pattern
            'attributes': [],
    ...
    def mount(self, opts):
        ...
        self._separator = self._options[
            'separator'] if 'separator' in self._options else os.sep

        #default file attribute
        self._defaults = {
            'read': self._options['defaults']['read'] if 'read' in self._options[
                'defaults'] else True,
            'write': self._options['defaults']['write'] if 'write' in self._options[
                'defaults'] else True,
            'locked': False,
            'hidden': False
        }
        ...
        #assign some options to private teams
        ...
        for opt in ['uploadMaxSize', 'archiveMaxSize']:
            if not isinstance(self._options[opt], int):
                try:
                    self._options[opt] = int(self._options[opt][:-1]) * units[
                        self._options[opt][-1].lower()]
                except (TypeError, KeyError):
                    self._options[opt] = 0

        self._root_name = self._basename(self._root) if not self._options['alias'] else \
            self._options['alias']
        ...
        if (not 'read' in root or not root['read']) and (
                    not 'write' in root and not root['write']):
        ...
        if 'read' in root and root['read']:
            ...
            if self._options['startPath']:
                try:
                    ...
                    if start['mime'] == 'directory' and start[
                        'read'] and not self._is_hidden(start):
        ...
        self._attributes.append({
            'pattern': '^%s$' % re.escape(
                '%s%s' % (self._separator, self._options['quarantine'])),
            'read': False,
            'write': False,
            'locked': True,
            'hidden': True
        ...
        if not self._options['quarantine'] or (
                    isdir and not os.access(self._quarantine, os.W_OK)):
    ...
    def _configure(self):
        ...
        if self._options['tmbPath']:
            ...
            self._attributes.append({
                'pattern': '^%s$' % re.escape(
                    '%s%s' % (self._separator, self._relpath(path))),
                'locked': True,
                'hidden': True
            })
    ...
    def options(self, hash_):
        ...
        contains the following keys:
            ...
            :archivers:    the available create and extract archives (e.g. tar,
            gzip etc.)
        ...
        return {
            ...
            #this custom attribute (not part of the original response) is used to
            # return the hash url
            'pathUrl': '%s%s' % (self._options['URL'],
                                 self._relpath(path).replace(self._options['separator'],
                                                             '/')),
    ...
    def mime_accepted(self, mime, mimes=[], empty=True):
        ...
        return mime == 'directory' or 'all' in mimes or 'All' in mimes or mime in \
               mimes or mime[
                        0:mime.find(
                            '/')] in mimes
    ...
    def tree(self, hash_='', deep=0, exclude=''):
        ...
        return dirs + self._get_tree(path, (deep - 1) if deep > 0 else (
            self._options['treeDeep'] - 1), excluded)
    ...
    def tmb(self, hash_):
        ...
        if s[0] <= tmb_size and s[1] <= tmb_size:
            self._img_square_fit(im, tmb, tmb_size, tmb_size,
                                 self._options['tmbBgColor'], 'png')
        elif self._options['tmbCrop']:
            #Resize and crop if image bigger than thumbnail
            if s[0] > tmb_size and s[1] > tmb_size:
                resized = self._img_resize(im, None, tmb_size, tmb_size, True, False,
                                           'png')
                s = resized.size
                self._img_crop(resized, tmb, tmb_size, tmb_size,
                               int((s[0] - tmb_size) / 2), int((s[1] - tmb_size) / 2),
                               'png')
            else:
                fit = self._img_square_fit(im, None, s[0] if s[0] > s[1] else s[1],
                                           s[0] if s[0] > s[1] else s[1],
                                           self._options['tmbBgColor'], 'png')
                self._img_crop(fit, tmb, tmb_size, tmb_size,
                               int((s[0] - tmb_size) / 2) if s[0] > tmb_size else 0,
                               int((s[1] - tmb_size) / 2) if s[1] > tmb_size else 0,
                               'png')
        else:
            try:
    ...
    def mkdir(self, hash_dst, name):
        ...
        try:
            ...
            raise NamedError(ElfinderErrorMessages.ERROR_TRGDIR_NOT_FOUND,
                             '#%s' % error_path)
    ...
    def rename(self, hash_, name):
        ...
        if self.command_disabled('rename') or not self._attr(self._join_path(dir_, name),
                                                             'write'):
            raise PermissionDeniedError
        ...
        #path may not be a dir, _clear_cached_dir() will just fail on the dir key and
        # clear the file stat anyway
        self._clear_cached_dir(path)
    ...
    def upload(self, uploaded_file, hash_dst):
        ...
        if self._options['uploadOrder'][
            0].lower() == 'allow':  #['allow', 'deny'], default is to 'deny'
        ...
        if self._options['uploadMaxSize'] > 0 and uploaded_file.size > self._options[
            'uploadMaxSize']:
        ...
        try:
            ...
            if self._options['uploadOverwrite']:
                if not file_['write']:
                    ...
                    raise NamedError(ElfinderErrorMessages.ERROR_NOT_REPLACE,
                                     uploaded_file.name)
                self.remove(test)
            else:
    ...
    def paste(self, volume, hash_src, dst, rm_src=False):
        ...
        if self.command_disabled('paste') or (
                            volume == self and rm_src and self.command_disabled(
                        'rm')) or (
                            volume != self and rm_src and volume.command_disabled('rm')):
            raise PermissionDeniedError
        ...
        if file_['mime'] == 'directory' and (
                    (volume == self and rm_src and self.command_disabled('rmdir')) or (
                                    volume != self and rm_src and volume
                                .command_disabled(
                                'rmdir'))):
            raise PermissionDeniedError
        ...
        if test:
            raise NamedError(ElfinderErrorMessages.ERROR_LOCKED,
                             volume.path(test)) if rm_src else PermissionDeniedError
        ...
        try:
            ...
            if self._options['copyOverwrite']:
                ...
                if (file_['mime'] == 'directory' and stat['mime'] != 'directory') or (
                                file_['mime'] != 'directory' and stat[
                            'mime'] == 'directory'):
                    raise NamedError(ElfinderErrorMessages.ERROR_NOT_REPLACE,
                                     self._path(test))
                ...
                if volume == self and (('target' in file_ and test == file_[
                    'target']) or test == self.decode(hash_src)):
    ...
    def extract(self, hash_):
        ...
        archiver = self._archivers['extract'][file_['mime']] if file_['mime'] in \
                                                                self._archivers[
                                                                    'extract'] else False
        if not archiver:
    ...
    def archive(self, hashes, mime):
        ...
        archiver = self._archivers['create'][mime] if mime in self._archivers[
            'create'] else False
        if not archiver:
        ...
        name = '%s.%s' % (
            self._basename(files[0]) if len(files) == 1 else 'Archive', archiver['ext'])
    ...
    def resize(self, hash_, width, height, x, y, mode='resize', bg='', degree=0):
        ...
        if mode == 'propresize':
            ...
            self._img_square_fit(im, path, width, height,
                                 bg if bg else self._options['tmbBgColor'])
        elif mode == 'rotate':
    ...
    def encode(self, path):
        ...
        """
        if path:
            #cut ROOT from path for security reason, even if hacker decodes the path
            # he will not know the root
            #files hashes will also be valid, even if root changes
            p = self._relpath(path)
            #if reqesting root dir path will be empty, then assign '/' as we cannot
            # leave it blank for crypt
            if not p:
                p = self._separator
    ...
    def stat(self, path):
        ...
        if stat_cache is None or root_cache != self._root:
                ...
                if stat['mime'] == 'directory':  #handle directories
                    if self._options['checkSubfolders']:
                        if 'dirs' in stat:
                            if not stat['dirs']:
                                del stat['dirs']
                        elif 'alias' in stat and stat['alias'] and 'target' in stat and \
                                stat['target']:
                            stat['dirs'] = int('dirs' in stat_cache[stat['target']]) if \
                                stat['target'] in stat_cache else int(
                                self._subdirs(stat['target']))
                    ...
                    if not 'tmb' in stat and self._can_create_tmb(path, stat):
                        stat['tmb'] = self._get_tmb(
                            stat['target'] if 'target' in stat else path, stat)
            ...
            if root_cache != self._root:
                cache.set('elfinder::stat::%sroot' % self.id(), self._root,
                          60 * 60 * 24 * 10)
    ...
    def _attr(self, path, attr, val=False):
        ...
        if self._options['accessControl'] and hasattr(self._options['accessControl'],
                                                      '__call__'):
        ...
        for attrs in self._attributes:
            if attr in attrs and re.search(attrs['pattern'], '%s%s' % (
                    self._separator, self._relpath(path))):
                return attrs[attr]
    ...
    def _size(self, path):
        ...
        for stat in self._get_scandir(path):
            size = self._size(self._join_path(path, stat['name'])) if stat[
                                                                          'mime'] == \
                                                                      'directory' and \
                                                                      stat['read'] else \
                stat['size']
    ...
    def _get_tree(self, path, deep, exclude=''):
        ...
        for p in self._get_cached_dir(path):
            stat = self.stat(p)
            if not self._is_hidden(stat) and p != exclude and stat[
                'mime'] == 'directory':
                dirs.append(stat)
        ...
        for p in self._get_cached_dir(path):
            ...
            if q in name:
                ...
                if self._options['URL'] and not 'url' in stat:
                    stat['url'] = self._options['URL'] + p[len(self._root) + 1:].replace(
                        self._separator, '/')
                result.append(stat)
    ...
    def move(self, src, dst, name):
        ...
        if not stat['read'] or (
                        stat['mime'] == 'directory' and self.command_disabled('rmdir')):
            raise PermissionDeniedError
    ...
    def _can_create_tmb(self, path, stat):
        ...
        """
        return self._tmb_path_writable and not path.startswith(
            self._options['tmbPath']) and stat['mime'].startswith('image')

    def _img_resize(self, im, target, width, height, keepProportions=False,
                    resizeByBiggerSide=True, destformat=None):
        """
    ...
    def _img_square_fit(self, im, target, width, height, bgcolor='#ffffff',
                        destformat=None):
        """
        Put image to square
        """
    ...
    def _checkArchivers(self):
        ...
        if 'create' in self._options['archivers']:
            for mime, archiver in self._options['archivers']['create'].items():
                ...
                if re.match(r'application/', mime) and inspect.isclass(conf) and hasattr(
                        conf, 'open') and callable(getattr(conf, 'open')) and hasattr(
                        conf, 'add') and callable(getattr(conf, 'add')) and hasattr(conf,
                                                                                    'close') and callable(
                        getattr(conf, 'close')):
        ...
        if 'extract' in self._options['archivers']:
            for mime, archiver in self._options['archivers']['extract'].items():
                ...
                if re.match(r'application/', mime) and inspect.isclass(conf) and hasattr(
                        conf, 'open') and callable(getattr(conf, 'open')) and hasattr(
                        conf, 'extractall') and callable(
                        getattr(conf, 'extractall')) and hasattr(conf,
                                                                 'close') and callable(
                        getattr(conf, 'close')):
    ...
    def _unique_name(self, dir_, name, suffix=' copy', check_num=True):
        ...
        ext = ''
        m = re.search(
            r'\.((tar\.(gz|bz|bz2|z|lzo))|cpio\.gz|ps\.gz|xcf\.(gz|bz2)|[a-z0-9]{1,4})$',
            name, re.IGNORECASE)
        if m:
            ext = '.%s' % m.group(1)
    ...
    def _abspath(self, path):
        ...
        """
        return self._root if path == self._separator else self._join_path(self._root,
                                                                          path)
    ...
    def _path(self, path):
        ...
        """
        return self._root_name if path == self._root else self._join_path(
            self._root_name, self._relpath(path))
    ...
    def _save_uploaded(self, uploaded_file, dir_, name, **kwargs):
        """
        Save the Django
        `UploadedFile <https://docs.djangoproject
        .com/en/dev/topics/http/file-uploads/#django.core.files.uploadedfile
        .UploadedFile>`_
