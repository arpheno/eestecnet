...
from apps.elfinder.utils.accesscontrol import fs_standard_access
...
ELFINDER_JS_URLS = {
    ...
    'b_jqueryui': '//ajax.googleapis.com/ajax/libs/jqueryui/1.8.22/jquery-ui.min-1.11'
                  '.0.js',
...
ELFINDER_CSS_URLS = {
    'a_jqueryui': '//ajax.googleapis.com/ajax/libs/jqueryui/1.8'
                  '.22/themes/smoothness/jquery-ui.css',
...
ELFINDER_LANGUAGES_ROOT_URL = getattr(settings, 'ELFINDER_LANGUAGES_ROOT_URL',
                                      '%selfinder/js/i18n/' % settings.STATIC_URL)

#The available language codes. A corresponding ELFINDER_LANGUAGES_ROOT_URL/elfinder.{
# ext}.js url must be available
ELFINDER_LANGUAGES = getattr(settings, 'ELFINDER_LANGUAGES',
                             ['ar', 'bg', 'ca', 'cs', 'de', 'el', 'es', 'fa', 'fr', 'hu',
                              'it', 'jp', 'ko', 'nl', 'no', 'pl', 'pt_BR', 'ru', 'tr',
                              'zh_CN'])
    ...
    'default': {
        ...
        'roots': [
            ...
            #},
            {
                ...
                #'tmbPath' : '.tmb',
                #Thumbnails dir URL. Set this if you're storing thumbnails outside the
                #  root directory
                #'tmbURL' : '',
                ...
                'uploadMaxSize': '128m',
                #if True - every folder will be check for children folders, otherwise
                ...
                #Allowed archive's mimetypes to create. Leave empty for all available
                # types.
                #'archiveMimes' : [],
                #Manual config for archivers. Leave empty for auto detect
                'archivers': {
                    #create archivers must be a dictionary containing a class
                    # implementing the open, add, close methods and the archiver's
                    # file extension
                    #they should operate like the python's built-in tarfile.TarFile
                    # classes
                    #http://docs.python.org/library/tarfile.html
                    #'create' : { 'ext' : 'rar', 'archiver' : MyRarArchiver },
                    #extract archiver class must implement the open, extractall and
                    # close methods
