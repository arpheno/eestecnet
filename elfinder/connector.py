...
from exceptions import ElfinderErrorMessages, VolumeNotFoundError, DirNotFoundError, \
    FileNotFoundError, NamedError, NotAnImageError
...
class ElfinderConnector:
    ...
    `elfinder connector api v2.0  <https://github
    .com/Studio-42/elFinder/wiki/Client-Server-API-2.0>`_. At the moment, it supports
    all elfinder commands except from ``netDrivers``.
    """
    ...
    _commands = {
        ...
        'dim': {'target': True},
        'resize': {'target': True, 'width': True, 'height': True, 'mode': False,
    ...
    def version(self, commit=False):
        """
        Get api version. The commit number refers to the corresponding official
        elfinder github commit number.
        """
    ...
    def commandExists(self, cmd):
        ...
        """
        return cmd in self._commands and hasattr(self, '_%s' % cmd) and callable(
    ...
    def execute(self, cmd, **kwargs):
        ...
        """
        if not self._loaded:
            return {'error': self.error(ElfinderErrorMessages.ERROR_CONF,
                                        ElfinderErrorMessages.ERROR_CONF_NO_VOL)}
        ...
        if 'removed' in result:
            ...
            #replace removed files info with removed files hashes and filter out
            # duplicates
        ...
        **Command**: Open a directory
        ...
        Return:
            An array with following elements:
                ...
                :files:        opened directory content [and dirs tree if 'tree'
                argument is ``True``]
                :api:          api version (if 'init' argument is ``True``)
                :uplMaxSize:   The maximum allowed upload size (if 'init' argument is
                ``True``)
                :error:        on failed
        ...
        try:
            ...
            if not init:
                return {
                'error': self.error(ElfinderErrorMessages.ERROR_OPEN, display_hash, e)}
            else:
        ...
        try:
            ...
            if not cwd['read'] and init:
                try:
                    ...
                    return {
                    'error': self.error(ElfinderErrorMessages.ERROR_OPEN, display_hash,
                                        e)}
        except (DirNotFoundError, FileNotFoundError) as e:
            if init:
                cwd = volume.dir(hash_=volume.default_path(), resolve_link=True)
        ...
        if not cwd['read']:
            return {'error': self.error(ElfinderErrorMessages.ERROR_OPEN, display_hash,
                                        ElfinderErrorMessages.ERROR_PERM_DENIED)}
        ...
        try:
            ...
            return {
            'error': self.error(ElfinderErrorMessages.ERROR_OPEN, cwd['name'], e)}
    ...
    def _ls(self, target):
        ...
        **Command**: Return a directory's file list. This method should not be invoked
        ...
        try:
            ...
            return {
            'error': self.error(ElfinderErrorMessages.ERROR_OPEN, '#%s' % target)}

    def _tree(self, target):
        """
        **Command**: Return subdirs for required directory. This method should not be
        invoked
        directly, the :meth:`elfinder.connector.ElfinderConnector.execute`
        method must be used.
        """
    ...
    def _parents(self, target):
        """
        **Command**: Return parents dir for required directory. this method should not
        be invoked
        directly, the :meth:`elfinder.connector.ElfinderConnector.execute`
        method must be used.
        """
        try:
            ...
            return {
            'error': self.error(ElfinderErrorMessages.ERROR_OPEN, u'#%s' % target)}

    def _tmb(self, targets):
        """
        **Command**: Return new automatically-created thumbnails list. This method
    ...
    def _file(self, target, request=None, download=False):
        ...
        **Command**: Get a file
        ...
        Return:
            An array containing an opened file pointer, the root itself and the
        ...
        except os.error:  #Normally this could raise a FileNotFoundError as well,
        # but we already checked this
            return {'error': _('File not found'), 'header': {'Status': 404}, 'raw': True}
        ...
        if download:
            ...
            disp = 'inline' if re.match('(image|text)', file_['mime'], re.IGNORECASE) \
                or \
                               file_[
                                   'mime'] == 'application/x-shockwave-flash' else \
                'attachment'
            mime = file_['mime']

        filenameEncoded = urllib.quote(
            file_['name'].encode('utf-8'))  #unicode filename support
        if not '%' in filenameEncoded:  #ASCII only
            ...
            if re.search('MSIE [4-8]',
                         ua):  #IE < 9 do not support RFC 6266 (RFC 2231/RFC 5987)
        ...
        result = {
            ...
            'header': {
                ...
                'Content-Location': file_['name'].encode('utf-8'),
    ...
    def _mkdir(self, target, name):
        ...
        **Command**: Create a new directory. This method should not be invoked
        ...
        try:
            ...
            return {'error': self.error(ElfinderErrorMessages.ERROR_MKDIR, name,
    ...
    def _mkfile(self, target, name):
        ...
        **Command**: Create a new, empty file. This method should not be invoked
        ...
        try:
            ...
            return {'error': self.error(ElfinderErrorMessages.ERROR_MKFILE, name,
                                        ElfinderErrorMessages.ERROR_TRGDIR_NOT_FOUND,
                                        '#%s' % target)}
        ...
        try:
            ...
            return {
            'error': self.error(e, e.name, ElfinderErrorMessages.ERROR_MKFILE, name)}
        except Exception as e:
    ...
    def _rename(self, target, name):
        ...
        **Command**: Rename a file. This method should not be invoked
        ...
        try:
            ...
            return {
            'error': self.error(ElfinderErrorMessages.ERROR_RENAME, '#%s' % target,
                                ElfinderErrorMessages.ERROR_FILE_NOT_FOUND)}
        ...
        **Command**: Duplicate a file. Create a copy with "{suffix} %d" suffix,
        ...
        for target in targets:
            try:
                ...
                result['warning'] = self.error(ElfinderErrorMessages.ERROR_COPY,
                                               u'#%s' % target,
        ...
        **Command**: Remove directories or files. This method should not be invoked
        ...
        for target in targets:
            try:
                ...
                result['warning'] = self.error(ElfinderErrorMessages.ERROR_RM,
                                               '#%s' % target,
                                               ElfinderErrorMessages
                                               .ERROR_FILE_NOT_FOUND)
                continue
    ...
    def _paste(self, targets, dst, cut=False):
        ...
        **Command**: Copy/move ``targets`` files into a new destination ``dst``.
        ...
        error = ElfinderErrorMessages.ERROR_MOVE if cut else ElfinderErrorMessages\
            .ERROR_COPY
        ...
        try:
            ...
            return {'error': self.error(error, u'#%s' % targets[0],
                                        ElfinderErrorMessages.ERROR_TRGDIR_NOT_FOUND,
                                        u'#%s' % dst)}
        ...
        for target in targets:
            try:
                ...
                result['warning'] = self.error(error, u'#%s' % target,
                                               ElfinderErrorMessages
                                               .ERROR_FILE_NOT_FOUND)
                continue
    ...
    def _get(self, target):
        ...
        **Command**: Return file contents. This method should not be invoked
        ...
        try:
            ...
            return {
            'error': self.error(ElfinderErrorMessages.ERROR_OPEN, volume.path(target),
                                e)}
        ...
        try:
            ...
            return {'error': self.error(ElfinderErrorMessages.ERROR_NOT_UTF8_CONTENT,
                                        volume.path(target))}
    ...
    def _put(self, target, content):
       ...
       **Command**:  Save ``content`` into a text file. This method should not be invoked
        ...
        try:
            ...
            return {
            'error': self.error(ElfinderErrorMessages.ERROR_SAVE, u'#%s' % target,
                                ElfinderErrorMessages.ERROR_FILE_NOT_FOUND)}
        ...
        try:
            ...
            return {
            'error': self.error(ElfinderErrorMessages.ERROR_SAVE, volume.path(target),
                                e)}
    ...
    def _extract(self, target):
        ...
        **Command**:  Extract files from archive. This method should not be invoked
        ...
        try:
            ...
            return {
            'error': self.error(ElfinderErrorMessages.ERROR_EXTRACT, u'#%s' % target,
                                ElfinderErrorMessages.ERROR_FILE_NOT_FOUND)}
        ...
        try:
            ...
            return {
            'error': self.error(ElfinderErrorMessages.ERROR_EXTRACT, volume.path(target),
                                e)}
    ...
    def _archive(self, targets, type_):
        ...
        **Command**: Create a new archive file containing all files in
        ...
        try:
            ...
            return {'error': self.error(ElfinderErrorMessages.ERROR_ARCHIVE,
                                        ElfinderErrorMessages.ERROR_TRGDIR_NOT_FOUND)}
    ...
    def _info(self, targets, options=False):
        """
        **Command**: Return file info (used by client "places" ui). This method should
        not be invoked
        directly, the :meth:`elfinder.connector.ElfinderConnector.execute`
        method must be used.
        """
    ...
    def _resize(self, target, width, height, mode=None, x='0', y='0', degree='0'):
        ...
        **Command**: Resize ``target`` image. This method should not be invoked
        ...
        method must be used.
        """
        width, height, x, y, degree = int(width), int(height), int(x), int(y), int(
            degree)
        bg = ''
        ...
        try:
            ...
            return {
            'error': self.error(ElfinderErrorMessages.ERROR_RESIZE, '#%s' % target,
                                ElfinderErrorMessages.ERROR_FILE_NOT_FOUND)}

        try:
            return {
            'changed': [volume.resize(target, width, height, x, y, mode, bg, degree)]}
        except Exception as e:
            return {
            'error': self.error(ElfinderErrorMessages.ERROR_RESIZE, volume.path(target),
                                e)}
