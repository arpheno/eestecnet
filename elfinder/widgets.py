...
class ElfinderWidget(Input):
    ...
    ``optionset``
        The key of the ELFINDER_CONNECTOR_OPTION_SETS setting to use as connector
        settings
    """
    input_type = 'hidden'
    ...
    def _media(self):
        ...
        """
        js = [ls.ELFINDER_JS_URLS[x] for x in sorted(ls.ELFINDER_JS_URLS)] + [
            ls.ELFINDER_WIDGET_JS_URL]
        screen_css = [ls.ELFINDER_CSS_URLS[x] for x in sorted(ls.ELFINDER_CSS_URLS)] + [
            ls.ELFINDER_WIDGET_CSS_URL]

        #add language file to javascript media
        if not self.current_locale.startswith(
                'en') and self.current_locale in ls.ELFINDER_LANGUAGES:
            js.append('%selfinder.%s.js' % (
            ls.ELFINDER_LANGUAGES_ROOT_URL, self.current_locale))
    ...
    def render(self, name, value, attrs=None):
        ...
        Display the widget
        """
        #if self.optionset in ls.ELFINDER_CONNECTOR_OPTION_SETS and 'uploadAllow' in
        # ls.ELFINDER_CONNECTOR_OPTION_SETS[self.optionset] and ls
        # .ELFINDER_CONNECTOR_OPTION_SETS[self.optionset]['uploadAllow']:
        #    html = '<div class="elfinder_filetypes">(' + _('Allowed mime types: ') +
        # str(ls.ELFINDER_CONNECTOR_OPTION_SETS[self.optionset]['uploadAllow']) +
        # ')</div>'
        ...
        html = ('%(super)s\n'
                ...
                '                %(file)s,\n'
                '                %(elfinder)s,\n'
                '                keywords : { size : "%(size)s", path : "%(path)s", '
                'link : "%(link)s", modified : "%(modified)s", dimensions : "%('
                'dimensions)s", update : "%(update)s", set : "%(set)s", clear : "%('
                'clear)s" }'
                '            });\n'
                '        })\n'
                '    })(yawdelfinder.jQuery)\n'
                '</script>' % {
                    ...
                    'file': file_,
                    'elfinder': elfinder,
                    #these keywords are optional, since they are initialized in
                    # elfinderwidget
